...
class AdminUser extends AdminBase
{
    ...
    {
        //$admin_user_list = $this->admin_user_model->alias('aum')->join('auth_group_access aga','aum.id=aga.uid','aga')->select();
        $join = [
            ['tq_auth_group_access aga','au.id=aga.uid'],
            ['tq_auth_group ag','aga.group_id=ag.id']
        ];
        $admin_user_list = Db::table('tq_admin_user')->field('au.*, ag.title')->alias('au')->join($join)->select();
    ...
    public function update($id, $group_id)
    {
        if ($this->request->isPost()) {
            ...
            if ($validate_result !== true) {
                ...
                $admin_user->id       = $id;
                $admin_user->loginname = $data['loginname'];
